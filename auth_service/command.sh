...
docker run -d --name auth_service_stage --env-file .env.stage -p 6061:8080 auth_service:stage
docker ps -a
...
docker run -d --name auth_service_prod --env-file .env.prod -p 6062:8080 auth_service:prod
docker ps -a

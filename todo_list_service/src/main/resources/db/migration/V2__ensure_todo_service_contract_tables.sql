-- V2__ensure_todo_service_contract_tables.sql
-- Idempotent repair migration for databases that were baselined before V1 ran.
-- Keeps all todo_list_service-owned objects inside the todo schema.
...
create table if not exists todo.todo_history (
  id text primary key,
  todo_id text not null references todo.todos(id) on delete cascade,
  user_id text not null,
  actor_id text,
  actor_role text,
  tenant text not null,
...
create table if not exists todo.outbox_events (
...
create index if not exists idx_outbox_pending
  on todo.outbox_events(status, next_retry_at, created_at);

create table if not exists todo.kafka_inbox_events (
  id uuid primary key default gen_random_uuid(),
  event_id text not null unique,
  tenant text,
  topic text not null,
...
create unique index if not exists idx_kafka_inbox_topic_partition_offset
  on todo.kafka_inbox_events(topic, partition, offset_value);

create table if not exists todo.access_grant_projections (

...
docker run -d --name todo_list_service_dev --env-file .env.dev -p 3030:8080 todo_list_service:dev
docker ps -a


# STAGE
docker build -t todo_list_service:stage .
docker run -d --name todo_list_service_stage --env-file .env.stage -p 3031:8080 todo_list_service:stage
docker ps -a
...
docker run -d --name todo_list_service_prod --env-file .env.prod -p 3032:8080 todo_list_service:prod
docker ps -a

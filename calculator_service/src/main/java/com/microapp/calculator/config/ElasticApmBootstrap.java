...
public final class ElasticApmBootstrap {
    ...
    private static final String APPLICATION_PACKAGES = "com.microapp.calculator";
    ...
    public static void attachFromEnvironment() {
        Map<String, String> config = baseConfig(
                env("CALC_SERVICE_NAME", "ELASTIC_APM_SERVICE_NAME", "calculator_service"),
                env("CALC_VERSION", "ELASTIC_APM_SERVICE_VERSION", "v1.0.0"),
                displayEnvironment(env("CALC_ENV", "ELASTIC_APM_ENVIRONMENT", "development")),
                env("CALC_APM_SERVER_URL", "ELASTIC_APM_SERVER_URL", ""),
                env("CALC_APM_SECRET_TOKEN", "ELASTIC_APM_SECRET_TOKEN", ""),
                env("CALC_APM_TRANSACTION_SAMPLE_RATE", "ELASTIC_APM_TRANSACTION_SAMPLE_RATE", "1.0"),
                env("CALC_APM_CAPTURE_BODY", "ELASTIC_APM_CAPTURE_BODY", "errors"),
                env("CALC_TENANT", null, "dev")
        );
        attach(config);
    }

    public static void attach(AppProperties props) {
        Map<String, String> config = baseConfig(
                props.getServiceName(),
                props.getVersion(),
                displayEnvironment(props.getEnvironment()),
                props.getApm().getServerUrl(),
                props.getApm().getSecretToken(),
                props.getApm().getTransactionSampleRate(),
                props.getApm().getCaptureBody(),
                props.getTenant()
        );
        attach(config);
    }
